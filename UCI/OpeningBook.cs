...
{
    /// <summary>
    /// Polyglot .bin opening book reader. Compatible with Cerebellum 3 Merge and other Polyglot books.
    /// Uses the standard Polyglot Zobrist key (781-value table) for position lookup.
    /// </summary>
    public static class OpeningBook
    {
        ...
        private static int _bookDepth = 36;   // max plies to use book (18 full moves)
        private static int _bookEvalLimit = -40; // exit book if eval < -40 cp (-0.4 pawns)

        private static byte[]? _bookData;
        private static int _entryCount;

        /// <summary>Path to the .bin book file. Empty = disabled.</summary>
        ...
        /// <summary>Max plies (half-moves) to use book. Default 36 = 18 full moves.</summary>
        public static int BookDepth { get => _bookDepth; set => _bookDepth = Math.Clamp(value, 0, 60); }

        /// <summary>Exit book if static eval &lt; this (centipawns, side-to-move). Default -40.</summary>
        public static int BookEvalLimit { get => _bookEvalLimit; set => _bookEvalLimit = value; }

        /// <summary>
        /// Try to get a book move for the position. Returns null if book disabled, no entries, eval below limit, or beyond book depth.
        /// Uses weighted random selection among all book moves for the position.
        /// </summary>
        ...
        private static bool EnsureLoaded()
        {
            ...
            catch { _bookData = null; return false; }
        }

        private static List<(ushort rawMove, ushort weight)> FindEntries(ulong key)
        {
        ...
        private static ulong ReadKey(int index)
        {
            int offset = index * EntrySize;
            return ((ulong)_bookData![offset] << 56) | ((ulong)_bookData[offset + 1] << 48) |
                   ((ulong)_bookData[offset + 2] << 40) | ((ulong)_bookData[offset + 3] << 32) |
                   ((ulong)_bookData[offset + 4] << 24) | ((ulong)_bookData[offset + 5] << 16) |
                   ((ulong)_bookData[offset + 6] << 8) | _bookData[offset + 7];
        }

        private static ushort ReadRawMove(int index)
        {
            int offset = index * EntrySize + 8;
            return (ushort)((_bookData![offset] << 8) | _bookData[offset + 1]);
        }

        private static ushort ReadWeight(int index)
        {
            int offset = index * EntrySize + 10;
            return (ushort)((_bookData![offset] << 8) | _bookData[offset + 1]);
        }

        private static bool TryDecodeToLegal(ushort rawMove, Board board, List<Move> legal, out Move move)
        {
            int to = rawMove & 0x3F;
            int from = (rawMove >> 6) & 0x3F;
            int prom = (rawMove >> 12) & 0x7;

            Piece promotion = prom switch
            {
                1 => board.WhiteToMove ? Piece.WN : Piece.BN,
                2 => board.WhiteToMove ? Piece.WB : Piece.BB,
                3 => board.WhiteToMove ? Piece.WR : Piece.BR,
                4 => board.WhiteToMove ? Piece.WQ : Piece.BQ,
                _ => Piece.Empty
            };
        ...
        /// <summary>Compute Polyglot Zobrist key for the position. Uses standard 781-value table.</summary>
        public static ulong ComputePolyglotKey(Board board)
        {
            ulong hash = 0;
        ...
        // Polyglot piece index: WP->0, BP->1, WN->2, BN->3, WB->4, BB->5, WR->6, BR->7, WQ->8, BQ->9, WK->10, BK->11
        private static readonly int[] PolyglotPieceIndex = { -1, 0, 2, 4, 6, 8, 10, 1, 3, 5, 7, 9, 11 };

        // Standard Polyglot random array (781 values). From python-chess / Polyglot spec.
        private static readonly ulong[] PolyglotRandom =
        {
            ...
            0x08B889D624D44885D, 0xF4D14597E660F855, 0xD4347F66EC8941C3, 0xE699ED85B0DFB40D,
            ...
            0x2738259634305C14, 0x61CF4F94C97DF93D, 0x1B6BACA2AE4E125B, 0x0758F450C88572E0B,
